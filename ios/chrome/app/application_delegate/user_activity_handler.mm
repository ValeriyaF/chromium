...
#import "ios/chrome/app/intents/RunJSCodeIntent.h"
...
NSString* const kSiriShortcutRunJSCode = @"RunJSCodeIntent";
  ...
  if ([userActivity.activityType
    ...
    webpageURL =
        [NSURL URLWithString:base::SysUTF8ToNSString(kChromeUINewTabURL)];
              ...
              isEqualToString:kSiriShortcutRunJSCode]) {
      RunJSCodeIntent* intent = base::mac::ObjCCastStrict<RunJSCodeIntent>(
          userActivity.interaction.intent);
      NSString* jsCode = intent.code;

      if ((jsCode != nil) && (browserState->GetWebStateList()->GetActiveWebState() != nil)) {
